    ...
    return (
        <div>
            ...
            {filteredData.map(name=>(
                ...
                <img style={{display:'inline-block'}} width="300" height="300" src={'/content/'+name.propic} ></img>
                <a style={{fontSize:'42px', marginLeft:'30%', display:'inline-block'}} href='#' onClick={()=>{history.push('/mechanicprofile/'+name._id);}} >{name.firstname} {name.lastname}</a><br></br>
                        <a style={{ marginLeft:'58%', backgroundColor:'#f6003c', borderRadius:'25%', color:'#ffffff'}}>Email: {name.email}</a><br></br>
                        <a style={{marginLeft:'60%', backgroundColor:'#fff591', borderRadius:'25%', color:'#e41749'}}> <b>Works from</b> {name.startinghours} to {name.endinghours}</a>
                </div>
            </div> 
            ))}
        </div>
